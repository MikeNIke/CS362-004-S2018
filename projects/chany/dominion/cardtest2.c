...
int main() {
    ...
    cardEffect(smithy, choice1, choice2, choice3, &G, hp, &bonus);

    printf("\tPlayer0 gains 3 cards:\n");
    assertTrue(G.handCount[player0],OG.handCount[player0]+2,&err_cnt);

    printf("\tPlayer0 gains 3 cards from his own pile:\n");
    assertTrue(G.deckCount[player0],OG.deckCount[player0]-3,&err_cnt);
